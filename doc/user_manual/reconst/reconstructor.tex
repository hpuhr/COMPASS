...
A common Tracked data source accuracy \textbf{scale factor} map can look as follows, with this source showing a distinct accuracy over-estimation (tracking accuracy vs. measured against reconstructed accuracy) in the southern region:
\begin{figure}[H]
    \hspace*{-2.5cm}
...
The (rescaled) average Tracked data source accuracy map can look as follows, showing a nice common accuracy region, with slightly increased errors in some regions (most probably airports) and in the outer limits.
\begin{figure}[H]
    \hspace*{-2.5cm}
