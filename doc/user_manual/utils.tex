...
Users are invited to report issues or requests for improvement
 however the provider has no obligation to process users' requests and
...
where the following options are supported:
 ...
 -list_065		list ASTERIX category 065 messages
 -list_dsi		list data source identifiers (SAC/SIC)
 -list_gh		list geometric height
 -list_lus		list last updating sensor (for system tracks)
 -list_only_bg		list only system tracks from background service
 -list_only_cy		list only system tracks from complementary service
 -list_service_id	list service identification (for system tracks)
 -ll=nn			length limit (only first nn bytes are listed)
 -ml			handle multiple lines (per sensor)
 -mof			list mode of flight (for system tracks)
 -nft			don't list frame time
 -no_utc		No UTC time of day in list file
 -progress		show some progress indication
 -reftrj		input file holds reference trajectories
 -soe			stop on (ASTERIX) error
 -sqn			with sequence numbers
 -srv_id=xxx		filter ARTAS output by service_identification
 -ssta			list sensor status information
 -start_date=yyyy-mm-dd	start date
 -start_offset=nnn	start offset in input file
 -subtypes		print list of supported format subtypes
 -timebias=ttt		time bias (milliseconds)
 -tn12			system track number with 12 bits (legacy)
...
If you need additional categories or versions, please send us an e-mail:
 sddl@gmx.at
