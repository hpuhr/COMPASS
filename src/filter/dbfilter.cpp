...
: Configurable (class_id, instance_id, parent), is_generic_(is_generic) // filter_manager_(*filter_manager),
{
    ...
    if (getClassId().compare("DBFilter")==0) // else do it in subclass
...
DBFilter::~DBFilter()
{
    logdbg  << "DBFilter: destructor: instance_id " << getInstanceId();
                                          ...
                                          std::vector <DBOVariable*>& filtered_variables)
{
    ...
    if (active_)
    {
        for (unsigned int cnt=0; cnt < conditions_.size(); cnt++)
        {
            ...
            {
                logwrn  << "DBFilter " << getInstanceId() << ": getConditionString: invalid condition, will be skipped";
                continue;
            }
    ...
    loginf  << "DBFilter " << getInstanceId() << ": getConditionString: object " << dbo_name << " here '" << ss.str()
            << "' first " << first;
...
{
    logdbg  << "DBFilter: generateSubConfigurable: " << getClassId() << " instance " << getInstanceId();
...
{
    logdbg  << "DBFilter: checkSubConfigurables: " << getClassId();

    if (!widget_)
    {
        logdbg  << "DBFilter: checkSubConfigurables: generating generic filter widget";
        widget_ = new DBFilterWidget ("DBFilterWidget", getInstanceId()+"Widget0", *this);
