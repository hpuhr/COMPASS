...
{
  loginf  << "FilterEditWidget: destructor";
}

void FilterEditWidget::createGUIElements ()
{
  QFont font_bold;
...
//  dbovars_grid_->addWidget (unit_label, 0, 5);
  ...
  for (it = conditions.begin(); it != conditions.end(); it++)
  {
    ...
    var_select->setSelectedVariable ((*it)->getVariable());
...
void FilterEditWidget::changedConditionVariable()
{
  ...
  conditions_variable_selects_[source]->setVariable(source->getSelectedVariable());
}
