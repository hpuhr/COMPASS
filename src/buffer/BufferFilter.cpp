...
@param parent Configurable parent.
  */
BufferFilterEntry::BufferFilterEntry( const std::string& class_id,
...
/**
Checks if the entries Property exists in the given buffer. Note that the data type is directly checked
for equality.
...
@return Property id as string.
  */
const std::string& BufferFilterEntry::getID() const
{
    return id_;
}
...
@param filter_rule Filter rule as integer number (see class BufferFilter).
  */
void BufferFilterRuleEntry::setRule( int filter_rule )
{
    filter_rule_ = filter_rule;
}
...
@return filter_rule Filter rule as integer number (see class BufferFilter).
  */
int BufferFilterRuleEntry::filterRule() const
{
    return filter_rule_;
}
...
@param parent Configurable parent.
  */
BufferFilter::BufferFilter( const std::string& class_id,
                            const std::string& instance_id,
                            Configurable* parent )
:   Configurable( class_id, instance_id, parent ),
    base_rule_( NULL )
...
/**
Adds one or more new filter properties to the filter using the given DBOVariable. If the variable is of meta type
all of the subvariables properties will be added to the filter. If the given Property id already exists for the a specific DBO type
an exception will be thrown.
...
/**
Sets the filter rule for the given DBO type. Note that rules that already exist for the given DBO type
will just get overwritten by new ones.
...
the given rule will be added for all defined subvariable DBO types. Note that rules that already exist
for a given DBO type will just get overwritten by new ones.
...
/**
Sets the filters base rule, which will be used if no rule is defined for a specific buffers DBO type.
...
@return True if a base filter rule is defined, false otherwise.
  */
bool BufferFilter::hasBaseRule() const
{
    return base_rule_ != NULL;
}

/**
Returns the base filter rule, which is used if no specific filter rule is defined for a certain buffer.
