...
MySQLServerWidget::MySQLServerWidget(MySQLppConnection& connection, MySQLServer& server,
                                     QWidget* parent)
    : QWidget(parent), connection_(connection), server_(server)
{
    ...
    connect(db_name_box_, SIGNAL(currentIndexChanged(const QString&)), this,
            SLOT(updateDatabaseSlot(const QString&)));
...
void MySQLServerWidget::newDatabaseSlot()
{
    ...
    bool ok;
    QString text = QInputDialog::getText(this, tr("QInputDialog::getText()"), tr("Database name:"),
...
void MySQLServerWidget::clearDatabaseSlot()
{
    ...
    question += "'.";
    reply = QMessageBox::question(this, "Clear Database", question.c_str(),
...
void MySQLServerWidget::deleteDatabaseSlot()
{
    ...
    question += server_.database().c_str();
    question += "'.";
    reply = QMessageBox::question(this, "Delete Database", question.c_str(),
                                  QMessageBox::Yes | QMessageBox::No);
...
void MySQLServerWidget::openDatabaseSlot()
{
    ...
    if (server_.database() == "")
    {
        QMessageBox m_warning(
            QMessageBox::Warning, "No Database Selected",
