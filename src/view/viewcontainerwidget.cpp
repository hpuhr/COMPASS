...
{
  logdbg  << "ViewContainerWidget: constructor: instance " << getInstanceId();
  ...
  name_ = "Window"+std::to_string(String::getAppendedInt (getInstanceId()));
...
void ViewContainerWidget::generateSubConfigurable (const std::string &class_id, const std::string &instance_id)
{
    if (class_id.compare ("ViewContainer") == 0)
    {
        ...
        view_container_ = new ViewContainer (class_id, instance_id, this, &view_manager_, tab_widget_,
                                             String::getAppendedInt(getInstanceId()));
        assert (view_container_);
    }
    else
...
void ViewContainerWidget::checkSubConfigurables ()
{
    if (!view_container_)
    {
        generateSubConfigurable ("ViewContainer", getInstanceId()+"ViewContainer0");
        assert (view_container_);
    }
}
...
{
  loginf  << "ViewContainerWidget: closeEvent: instance " << getInstanceId();
  view_manager_.deleteContainerWidget (getInstanceId());
  QWidget::closeEvent(event);
}

void ViewContainerWidget::moveEvent (QMoveEvent* event)
{
  logdbg  << "ViewContainerWidget " << getInstanceId() << ": moveEvent";
  pos_x_ = event->pos().x();
  pos_y_ = event->pos().y();
}
...
{
  logdbg  << "ViewContainerWidget " << getInstanceId() << ": resizeEvent";
