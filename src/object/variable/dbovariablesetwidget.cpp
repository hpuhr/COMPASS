...
void DBOVariableSetWidget::updateEntries()
{
    ...
    for (dobit = dobs.begin(); dobit != dobs.end(); dobit++)
    {
        ...
        logdbg << "DBOVariableSetWidget: updateEntries: looking for type " << type << " contained "
               << ATSDB::getInstance().contains(type);
        ...
        std::map<std::string, DBOVariable*>& vars =
            DBObjectManager::getInstance().getDBOVariables(type);
...
void DBOVariableSetWidget::showMenuSlot() { menu_.exec(QCursor::pos()); }
...
void DBOVariableSetWidget::updateVariableList()
{
    ...
    for (it = variables.begin(); it != variables.end(); it++)
    {
        ...
        list_widget_->addItem(
            (DBObjectManager::getInstance().getDBObject((*it)->getDBOType())->getName() + ", " +
             (*it)->getName())
                .c_str());
    }
}
