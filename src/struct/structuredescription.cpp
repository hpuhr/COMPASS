...
StructureDescription::StructureDescription(const std::string& dbo_type, string id,
                                           string description, size_t offset)
...
StructureVariable* StructureDescription::addStructureVariable(string id,
                                                              StructureElementDataType type,
                                                              ...
                                                              size_t offset)
{
    ...
    if (present_variable_)
        var->addPresentStructureVariable(present_variable_->getId(), present_variable_->getType(),
                                         present_variable_->getNumber(),
                                         present_variable_->getDescription(),
                                         present_variable_->getOffset());

    return var;
}

StructureDescription* StructureDescription::addStructureDescription(string id, string description,
                                                                    size_t offset)
{
    ...
    if (present_variable_)
        desc->addPresentStructureVariable(present_variable_->getId(), present_variable_->getType(),
                                          present_variable_->getNumber(),
                                          present_variable_->getDescription(),
                                          present_variable_->getOffset());
...
void StructureDescription::addToFlatStructureDescription(StructureDescription* flatdesc,
                                                         std::string prefix)
{
    if (id_.length() != 0)
...
unsigned int StructureDescription::getSize() { return element_list_.size(); }
...
//  Configuration &configuration = ConfigurationManager::getInstance().getConfiguration ("DBObject",
//  db_config_name, dboname); configuration.addParameter ("dbo_type", dbo_type_);
//  configuration.addParameter ("name", id_);
//  configuration.addParameter ("is_loadable", !((id_.compare("Sensor") == 0) ||
//  (id_.compare("Undefined") == 0)));
//
//  std::string id_variable_id = dboname+"id";
//  std::string id_variable_config_id=id_variable_id+"Config0";
//  logdbg  << "StructureDescription: createDBObject: adding element " << id_variable_id << " with
//  config "  << id_variable_config_id << " to dbo " << dboname << " with config "<< db_config_name;
//  Configuration &id_configuration = ConfigurationManager::getInstance().getConfiguration
//  ("DBOVariable", id_variable_config_id, id_variable_id); id_configuration.addParameter ("id",
//  (std::string) "id"); id_configuration.addParameter ("description", (std::string) "Unique id");
...
//    logdbg  << "StructureDescription: createDBObject: element " << cnt << " dbo type " <<
//    dbo_type_; if (element->isVariable())
//    {
...
//      Configuration &variable_configuration = ConfigurationManager::getInstance().getConfiguration
//      ("DBOVariable", variable_config_id, variable_id);
...
//      variable_configuration.addParameter ("data_type",
//      data_type_conversion_table_.at(variable->getType()));
...
//
//      logdbg  << "StructureDescription: createDBObject: adding element " << variable_id << " with
//      config "  << variable_config_id << " to dbo " << dboname << " with config "<<
//      db_config_name; configuration.addSubConfigurable ("DBOVariable", variable_id,
//      variable_config_id);
//    }
//  }
...
void StructureDescription::addPresentStructureVariable(std::string id,
                                                       StructureElementDataType type, int number,
                                                       std::string description, size_t offset)
{
    if (present_variable_ != 0)
    {
        logerr << "StructureDescription: addPresentStructureVariable: variable " << id_
               << " present old " << present_variable_->getId() << " superseded by " << id;
