...
StructureReader::StructureReader(DBInterface* db_interface) : db_interface_(db_interface)
{
    logdbg << "StructureReader: constructor";

    std::map<DB_OBJECT_TYPE, StructureDescription*>& descriptions =
        StructureDescriptionManager::getInstance().getStructureDescriptions();
...
void StructureReader::add(DB_OBJECT_TYPE type, void* data)
{
    ...
    for (unsigned int cnt = 0; cnt < size; cnt++)
    {
        ...
        logdbg << "StructureReader: add: var " << var->getNumber() << " " << var->getId()
               << " type " << var->getType() << " data add " << data << " off " << var->getOffset();
        // assumes that memory is cleared, which it is (ArrayTemplate), and that the same order is
        // in StructureDescription and PropertyList of PropertyContainer

        // if (var->getId().compare("radar_number") == 0)
        //  loginf  << "StructureReader: add: var " << var->getNumber() << " " << var->getId() << "
        //  type " << var->getType() << " data value " << *(short int*) ((unsigned char*)data +
        //  var->getOffset()) << " off " << var->getOffset();
        ...
        if (var->hasPresentVariable())
        {
            ...
            if (!present)
            {
                ...
                // loginf  << "StructureReader: add: got not present at type " << (int)type<< " var
                // " << var->getId();
                continue;
            }
        }
        ...
        if (var->getType() == SE_TYPE_VARCHAR)
        {
            ...
            logdbg << "StructureReader: add: varchar src '"
                   << *((char**)((unsigned char*)data + var->getOffset())) << "' dst '" << *tmp
                   << "' size " << tmp->size();
        }
        else if (var->getType() == SE_TYPE_VARCHAR_ARRAY)
        {
            ...
            logdbg << "StructureReader: add: varchararray src '" << (char*)data + var->getOffset()
                   << "' dst '" << *tmp << "' size " << tmp->size();
        }
        else
...
{
    // TODO
    //    WriteBufferDBJob *job = new WriteBufferDBJob (this, boost::bind(
    //    &StructureReader::writeBufferDone, this, _1 ),
