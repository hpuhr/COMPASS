 ...
 *
 * Reads a defined list of information from a DBO. Used to get detailed information about specific
 * DBO records for either labels or a listbox.
 */
class DBOInfoDBJob : public DBJob
{
  public:
    DBOInfoDBJob(JobOrderer* orderer, boost::function<void(Job*)> done_function,
                 ...
                 DB_OBJECT_TYPE type, std::vector<unsigned int> ids, DBOVariableSet read_list,
                 bool use_filters, std::string order_by_variable, bool ascending,
                 unsigned int limit_min = 0, unsigned int limit_max = 0, bool finalize = 0);
