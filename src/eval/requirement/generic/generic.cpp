...
Generic::Generic(const std::string& name, const std::string& short_name, const std::string& group_name,
                       float prob, COMPARISON_TYPE prob_check_type, EvaluationManager& eval_man)
    : ProbabilityBase(name, short_name, group_name, prob, prob_check_type, eval_man)
{
}

std::shared_ptr<EvaluationRequirementResult::Single> Generic::evaluate (
        const EvaluationTargetData& target_data, std::shared_ptr<Base> instance,
        const SectorLayer& sector_layer)
{
    logdbg << "EvaluationRequirementGeneric '" << name_ << "': evaluate: utn " << target_data.utn_;
...
//        ok = ret_pos.second;
        ...
        else if (cmp_res == ValueComparisonResult::Different)
        {
            ...
            throw runtime_error("EvaluationRequirementGeneric: evaluate: unknown compare result "
                                +to_string(cmp_res));
...
std::string Generic::valueName() const
{
    return value_name_;
}

std::string Generic::valueNamePlural() const
{
    return value_name_plural_;
}

std::string Generic::probabilityName() const
{
    return probability_name_;
}

std::string Generic::probabilityNameShort() const
{
    return probability_name_short_;
}

std::string Generic::valueNameShort() const
{
