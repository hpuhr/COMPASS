...
protected:
    ...
    virtual void generateDetails(EvaluationResultsReport::Section& utn_req_section);
    ...
    /// derive to get a custom annotation index => detail key mapping (default is to directly use the annotation index)
    boost::optional<DetailKey> detailKey(const QVariant& annotation) const;
    /// derive to implement custom iteration over details (default is to just iterate over the detail vector => no detail nesting)
    ...
    void createTargetAnnotations(nlohmann::json& annotations_json,
                                 TargetAnnotationType type,
                                 const boost::optional<DetailKey>& detail_key = boost::optional<DetailKey>(),
                                 bool add_ok_details_to_overview = true) const;
