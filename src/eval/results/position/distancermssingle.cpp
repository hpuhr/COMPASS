...
SinglePositionDistanceRMS::SinglePositionDistanceRMS(const std::string& result_id,
    ...
    :   SinglePositionBase("SinglePositionDistanceRMS", result_id, requirement, sector_layer, utn, target, eval_man, details,
                           num_pos, num_no_ref,num_pos_outside, num_pos_inside, num_comp_passed, num_comp_failed, values)
{
    update();
}

void SinglePositionDistanceRMS::update()
{
...
void SinglePositionDistanceRMS::addToReport (std::shared_ptr<EvaluationResultsReport::RootItem> root_item)
{
    logdbg << "SinglePositionDistanceRMS " <<  requirement_->name() <<": addToReport";
...
void SinglePositionDistanceRMS::addTargetToOverviewTable(shared_ptr<EvaluationResultsReport::RootItem> root_item)
{
...
void SinglePositionDistanceRMS::addTargetDetailsToTable (
...
void SinglePositionDistanceRMS::addTargetDetailsToTableADSB (
        EvaluationResultsReport::Section& section, const std::string& table_name)
{
    if (!section.hasTable(table_name))
    {
...
void SinglePositionDistanceRMS::addTargetDetailsToReport(shared_ptr<EvaluationResultsReport::RootItem> root_item)
{
...
void SinglePositionDistanceRMS::reportDetails(EvaluationResultsReport::Section& utn_req_section)
{
    if (!utn_req_section.hasTable(tr_details_table_name_))
        utn_req_section.addTable(tr_details_table_name_, 8,
                                 {"ToD", "NoRef", "PosInside", "DistanceRMS", "CP", "#CF", "#CP", "Comment"});
        ...
        for (auto& rq_det_it : getDetails())
        {
            ...
            utn_req_details_table.addRow(
                        { Time::toString(rq_det_it.timestamp()).c_str(),
                          ...
                          rq_det_it.getValue(DetailKey::Value),                 // "DistanceRMS"
...
bool SinglePositionDistanceRMS::hasViewableData (
...
std::unique_ptr<nlohmann::json::object_t> SinglePositionDistanceRMS::viewableData(
        const EvaluationResultsReport::SectionContentTable& table, const QVariant& annotation)
{
    ...
    else if (table.name() == tr_details_table_name_ && annotation.isValid())
    {
        ...
        loginf << "SinglePositionDistanceRMS: viewableData: detail_cnt " << detail_cnt;
...
std::unique_ptr<nlohmann::json::object_t> SinglePositionDistanceRMS::getTargetErrorsViewable (bool add_highlight)
{
...
void SinglePositionDistanceRMS::addAnnotations(nlohmann::json::object_t& viewable, bool overview, bool add_ok)
{
...
std::vector<Eigen::Vector3d> SinglePositionDistanceRMS::getGridValues(const std::string& layer) const
{
...
std::string SinglePositionDistanceRMS::reference(
        const EvaluationResultsReport::SectionContentTable& table, const QVariant& annotation)
{
...
{
    return make_shared<JoinedPositionDistanceRMS> (result_id, requirement_, sector_layer_, eval_man_);
}

EvaluationRequirement::PositionDistanceRMS* SinglePositionDistanceRMS::req ()
{
    EvaluationRequirement::PositionDistanceRMS* req =
            dynamic_cast<EvaluationRequirement::PositionDistanceRMS*>(requirement_.get());
    assert (req);
    return req;
}
