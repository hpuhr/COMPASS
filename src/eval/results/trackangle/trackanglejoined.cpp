...
{
    logdbg << "JoinedTrackAngle " <<  requirement_->name() <<": addToReport";

    if (!results_.size()) // some data must exist
    {
        logerr << "JoinedTrackAngle " <<  requirement_->name() <<": addToReport: no data";
        return;
    }

    logdbg << "JoinedTrackAngle " <<  requirement_->name() << ": addToReport: adding joined result";

    addToOverviewTable(root_item);
    addDetails(root_item);
}

void JoinedTrackAngle::addToOverviewTable(std::shared_ptr<EvaluationResultsReport::RootItem> root_item)
{
...
void JoinedTrackAngle::addDetails(std::shared_ptr<EvaluationResultsReport::RootItem> root_item)
{
    ...
    sec_det_table.addRow({"#NoRef [1]", "Number of updates w/o reference trackangles", num_no_ref_}, this);
    ...
    sec_det_table.addRow({"#NoTstData [1]", "Number of updates without tst trackangle data", num_no_tst_value_}, this);

    // along
    sec_det_table.addRow({"OMin [m/s]", "Minimum of trackangle offset",
                          String::doubleToStringPrecision(value_min_,2).c_str()}, this);
    sec_det_table.addRow({"OMax [m/s]", "Maximum of trackangle offset",
                          String::doubleToStringPrecision(value_max_,2).c_str()}, this);
    sec_det_table.addRow({"OAvg [m/s]", "Average of trackangle offset",
                          String::doubleToStringPrecision(value_avg_,2).c_str()}, this);
    sec_det_table.addRow({"OSDev [m/s]", "Standard Deviation of trackangle offset",
                          String::doubleToStringPrecision(sqrt(value_var_),2).c_str()}, this);
    sec_det_table.addRow({"OVar [m^2/s^2]", "Variance of trackangle offset",
...
bool JoinedTrackAngle::hasViewableData (
...
std::unique_ptr<nlohmann::json::object_t> JoinedTrackAngle::getErrorsViewable ()
{
...
bool JoinedTrackAngle::hasReference (
        const EvaluationResultsReport::SectionContentTable& table, const QVariant& annotation)
{
    if (table.name() == req_overview_table_name_)
        return true;
...
std::string JoinedTrackAngle::reference(
        const EvaluationResultsReport::SectionContentTable& table, const QVariant& annotation)
{
...
void JoinedTrackAngle::exportAsCSV()
{
