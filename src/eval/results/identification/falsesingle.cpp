...
std::unique_ptr<nlohmann::json::object_t> SingleIdentificationFalse::getTargetErrorsViewable(bool add_highlight)
{
...
void SingleIdentificationFalse::addAnnotations(nlohmann::json::object_t& viewable, bool overview, bool add_ok)
{
    ...
    for (auto& detail_it : getDetails())
    {
        auto is_not_ok = detail_it.getValueAsOrAssert<bool>(
                    EvaluationRequirementResult::SingleIdentificationFalse::DetailKey::IsNotOk);
...
std::vector<Eigen::Vector3d> SingleIdentificationFalse::getGridValues(const std::string& layer) const
{
    std::vector<Eigen::Vector3d> values;

    if (layer == requirement_->name())
    {
        values = getGridValuesBinary(EvaluationRequirementResult::SingleIdentificationFalse::DetailKey::IsNotOk, true);
    }
