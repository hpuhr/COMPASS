 ...
 *
 * Returns SQL strings and commands for database functions. All DBCommands and DBCommandLists have
...
class SQLGenerator
{
  public:
    ...
    std::string createDBUpdateStringBind(std::shared_ptr<Buffer> buffer,
                                         const DBTableColumn& key_col, std::string tablename);
    ...
    //    DBCommand *getDistinctStatistics (const std::string &dbo_type, DBOVariable *variable,
    ...
    std::string getInsertSectorStatement(const std::string& name, const std::string& layer_name,
    ...
    //    ids, DBOVariableSet read_list,
    //            bool use_filters, std::string order_by_variable, bool ascending, unsigned int
    //            limit_min=0, unsigned int limit_max=0);
    ...
    //    std::string getDeleteStatement (const DBTableColumn &column, const std::string &value,
    //    const std::string &filter); std::string getUpdateStatement (const DBTableColumn &column,
    //    const std::string &value, const std::string &new_value, const std::string &filter);

    //    /// @brief Minimum/Maximum select statement for one variable in one table
    //    std::string getMinMaxSelectStatement (const std::string &variable, const std::string&
    //    table, const std::string &condition="");
    //    /// @brief Minimum/Maximum select statment for a number of variables in one table
    //    std::string getMinMaxSelectStatements (const std::vector <std::string> &variables, const
    //    std::string &table); std::string getDistinctSelectStatement (const DBTableColumn &column,
    //    const std::string &filter_condition=""); std::string getShowDatabasesStatement ();
