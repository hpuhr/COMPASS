...
//            if (!checkPositionOffsetAcceptable(distance_m, tgt_est_std_dev, tr_est_std_dev))
//            {
//                tn2utn_[tr.ds_id_][tr.line_id_].erase(*tr.track_number_);
//                reset_tr_assoc = true;
//            }
    ...
    bool do_debug)
{
    ...
    if (secondary_verified)
    {
        ...
        //        if (mahalanobis_dist < reconstructor_.settings().max_mahalanobis_sec_verified_dist_)
        //        {
        //            return std::pair<bool, double> (true, mahalanobis_dist);
        ////            results[target_cnt] = tuple<bool, unsigned int, double>
        ////                (true, other.utn_, mahalanobis_dist);
        ////            assert (distance_m <= max_mahalanobis_sec_verified_dist * max_sum_est_std_dev);
        //        }
...
{
    //using namespace ReconstructorAssociatorBase;
