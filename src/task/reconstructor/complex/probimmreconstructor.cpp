...
void ProbIMMReconstructor::clearOldTargetReports()
{
    loginf << "ProbIMMReconstructor: clearOldTargetReports: remove_before_time " << Time::toString(remove_before_time_)
    ...
    for (auto ts_it = target_reports_.begin(); ts_it != target_reports_.end() /* not hoisted */; /* no increment */)
    {
        ...
        {
            //loginf << "ProbIMMReconstructor: clearOldTargetReports: removing " << Time::toString(ts_it->second.timestamp_);
            ts_it = target_reports_.erase(ts_it);
        }
        else
        {
            //loginf << "ProbIMMReconstructor: clearOldTargetReports: keeping " << Time::toString(ts_it->second.timestamp_);
    ...
    loginf << "ProbIMMReconstructor: clearOldTargetReports: size after " << target_reports_.size();
            ...
            //    loginf << "ProbIMMReconstructor: clearOldTargetReports: per ds removed_cnt " << removed_cnt
...
void ProbIMMReconstructor::createTargetReports()
{
    loginf << "ProbIMMReconstructor: createTargetReports: current_slice_begin " << Time::toString(current_slice_begin_);
    ...
    for (auto& buf_it : *accessor_)
    {
        ...
        for (unsigned int cnt=0; cnt < buffer_size; cnt++)
        {
            ...
            ts = tgt_acc.timestamp(cnt);
                    ...
                    //loginf << "ProbIMMReconstructor: createTargetReports: ts " << Time::toString(ts);
            ...
            else // update buffer_index_
            {
                ...
                if (!target_reports_.count(record_num))
                    logerr << "ProbIMMReconstructor: createTargetReports: missing prev ts " << Time::toString(ts);
...
std::map<unsigned int, std::map<unsigned long, unsigned int>> ProbIMMReconstructor::createAssociations()
{
    loginf << "ProbIMMReconstructor: createAssociations";
...
//    }
    ...
    loginf << "ProbIMMReconstructor: createAssociations: done with " << num_assoc << " associated";

    return associations;
}


void ProbIMMReconstructor::saveAssociations(
    std::map<unsigned int, std::map<unsigned long,unsigned int>> associations)
{
    loginf << "ProbIMMReconstructor: saveAssociations";
    ...
    for (auto& cont_assoc_it : associations) // dbcontent -> rec_nums
    {
        ...
        loginf << "ProbIMMReconstructor: saveAssociations: db content " << dbcontent_name;
        ...
        loginf << "ProbIMMReconstructor: saveAssociations: dcontent " << dbcontent_name
        ...
        loginf << "ProbIMMReconstructor: saveAssociations: dcontent " << dbcontent_name << " done";
    }
    ...
    loginf << "ProbIMMReconstructor: saveAssociations: done";
}

void ProbIMMReconstructor::saveReferences()
{
    loginf << "ProbIMMReconstructor: saveReferences: num " << targets_.size();
    ...
    {
        loginf << "ProbIMMReconstructor: saveReferences: buffer size " << buffer->size();

        cont_man.insertData({{buffer->dbContentName(), buffer}});
    }
    else
        loginf << "ProbIMMReconstructor: saveReferences: empty buffer";
}

void ProbIMMReconstructor::saveTargets()
{
    loginf << "ProbIMMReconstructor: saveTargets: num " << targets_.size();
    ...
    loginf << "ProbIMMReconstructor: saveTargets: done";
}
