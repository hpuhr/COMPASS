...
void RS2GProjection::generateSubConfigurable (const std::string& class_id, const std::string& instance_id)
{
...
bool RS2GProjection::hasCoordinateSystem (unsigned int id)
{
    return coordinate_systems_.count(id);
}

void RS2GProjection::addCoordinateSystem (unsigned int id, double latitude_deg, double longitude_deg, double altitude_m)
{
    assert (!hasCoordinateSystem(id));

    coordinate_systems_[id].reset(new RS2GCoordinateSystem(id, latitude_deg, longitude_deg, altitude_m));
}


bool RS2GProjection::polarToWGS84 (unsigned int id, double azimuth_rad, double slant_range_m, bool has_baro_altitude,
